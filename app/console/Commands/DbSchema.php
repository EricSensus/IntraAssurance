...
class DbSchema extends Command {
    ...
    protected function configure() {
        ...
        $this->setName('db:schema')
                ->setDescription('Loads the Jenga Schema and runs the schema within the specified element')
                ->addArgument('element_schema', 
                        InputArgument::REQUIRED, 
                        'Specify the element schema using format <element>/<schema class> ')
                ->addOption('task', null, 
                        InputOption::VALUE_REQUIRED, 
                        'Select task to be executed. Default options are build, seed or run');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $element_schema = explode('/',$input->getArgument('element_schema'));
