<?php
//This namespace should end with the folder showing the db type. The default is Mysqli
namespace Jenga\App\Database\Mysqli;
...
class Database //directly builds from Config
{
    ...
    public $_where = array();
    /**
     * A string that holds the Distinct value in the query
     * 
     * @var boolean
     */
    protected $_distinct;
    ...
    /**
     * Variable which holds an amount of returned rows during get/find/select queries
     *
     * @var string
     */ 
    public $count = 0;
    ...
    /**
     * @Inject({"db","dbprefix","host","username","password","port"})    
     * @return \Jenga\App\Database\Mysqli\Database
     */
    public function __construct($db, $dbprefix, $host, $username, $password, $port){
        
        $this->config = new \stdClass();
        $this->_compileConfigurations($db, $dbprefix, $host, $username, $password, $port);
        
        if($this->config->port == NULL)
            $this->config->port = ini_get ('mysqli.default_port');

        if ($this->config->host == null && $this->config->username == null && $this->db == null) {
    ...
    /**
     * Compiles the configuration settings sent by the application
     * 
     * @param type $db
     * @param type $dbprefix
     * @param type $host
     ...
     * @return \Jenga\App\Database\Mysqli\Database
     */
    private function _compileConfigurations($db, $dbprefix, $host, $username, $password, $port){
     ...
     *
     * @uses $this->db = MySqliDb::getInstance();
     *
    ...
    public function setPrefix(){
        ...
        $this->prefix = $this->config->dbprefix;
        define('TABLE_PREFIX',$this->config->dbprefix);

        return $this;
    }

    /**
     * Pass in a raw query and an array containing the parameters to bind to the prepaired statement.
     *
    ...
    public function get($tableName, $numRows = null, $columns = '*'){
        ...
        $this->_query = "SELECT ".$distinct.$column." FROM " .$this->prefix . $tableName;
     ...
     * @return array Contains the returned rows from the select query.
     */
    public function find($tableName, $columns = '*') {
    ...
    public function update($tableName, $tableData)
    {
        ...
        $this->_query = "UPDATE " . $this->prefix . $tableName ." SET ";
     ...
     */
     public function join($joinTable, $joinCondition, $joinType = '')
     {
        ...
        $this->_join[$joinType . " JOIN " . $this->prefix . $joinTable] = $joinCondition;
    ...
    /**
     * Helper function to add variables into bind parameters array and will return its SQL part of the query according to operator in ' $operator ?' or
    ...
    */
     ...
     */
    public static function subQuery($modelname)
    {
