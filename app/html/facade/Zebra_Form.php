...
class Zebra_Form{
    ...
    /**
     * Reconfigures the input controlsto conmpily with the engine value sent
     * @var string 
     */
    public $engine;
     ...
     *  @return void
     */
    function __construct($name, $method = 'POST', $action = '', $attributes = '', $map = '', $engine = '')
    {
        ...
        require_once dirname(__FILE__) . '/Zebra_Form/includes/XSSClean.php';

        // include the Control.php file which contains the Zebra_Form_Control class which is
        // extended by all of the classes
        require_once dirname(__FILE__) . '/Zebra_Form/includes/Control.php';
    ...
    function &add($type)
    {
        ...
        // for all other controls
            ...
            require_once dirname(__FILE__) .'/Zebra_Form/includes/' . $file_name . '.php';
    ...
    function language($language)
    {
        ...
        require rtrim(dirname(__FILE__), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'Zebra_Form/languages/' . strtolower(trim($language)) . '.php';
     ...
     */
    function cede($template = '', $return = false, $variables = ''){
        ...
        elseif(is_array($template)){
            ...
            elseif($tmp['orientation'] == 'vertical'){
                $template = '*vertical';
            }
        }
        ...
        if (isset($this->errors['error_spam']) || isset($this->errors['error_csrf'])) {

            // if there's a CSRF error leave only that
            if (isset($this->errors['error_csrf'])) $this->errors['error'] = $this->errors['error_csrf'];

            // else, if there's a SPAM error, leave just that
            else $this->errors['error'] = $this->errors['error_spam'];

            // remove these two errors
            // as now there will be only one, under the name of "error"
            unset($this->errors['error_csrf']);
            unset($this->errors['error_spam']);
                ...
                foreach ($blocks as $controls) {
                    ...
                    elseif($this->engine == 'bootstrap'){
                        ...
                        if($tmp['orientation'] == 'horizontal'){
                            ...
                            if(isset($tmp['columns'])){
                                
                                $columns = explode(',', $tmp['columns']);
                                $contents .= '<div class="'.$columns[0].'">';
                            }
                        }
                    }
    ...
    function validate()
    {
        ...
        if (
            ...
            ) $this->add_error('error_spam', $this->form_properties['language']['spam_detected']);

            // else, if a possible CSRF attack was detected
            // show the appropriate error message to the user
            elseif (!$csrf_status) $this->add_error('error_csrf', $this->form_properties['language']['csrf_detected']);
