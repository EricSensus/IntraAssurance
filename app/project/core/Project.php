...
class Project implements ArgumentResolverInterface {
    ...
    private function _defaultElement(){
        ...
        foreach(self::$elements as $element){
            ...
            if(array_key_exists('default',$element)){
                ...
                $this->defaults['element'] = $elementNames[ $count ];                
    ...
    public function init(Request $request, \JRequestEvent &$event){
       ...
       //set the entire Authorization mechanism into the shell        
        ...
        elseif($controller == '_static'){
            ...
            App::set('view',\DI\object('Jenga\App\Views\View')->constructor($this->resources)); 
    ...
    public function assignMVC($controller, $model, $view, $method = null, $params = null){
        ...
        App::set( 'model', \DI\object($model));
        App::set( 'view', \DI\object($view)->constructor($this->resources) );
    ...
    public function makeMvcEngine($element, $controller = null) {
        ...
        if(!is_null($element)){
            ...
            //set correct model
            if($ctrposition !== FALSE){
                $modelname = $mkey[$ctrposition];
                
                //fall back in case the mvc components dont align
    ...
    public function makePartialMvcEngine($element, $controller){
        ...
        if(count($element['controllers'][$controller]) == 1)
            ...
            $engine['controller'] = $nsClass .'\\'. $element['controllers'][$controller]['folder'] .'\\'. $controller;
        
        //create the model
        if(count($element['models'][$model]) == 1)
            $engine['model'] = $nsClass .'\\'. $model;
        else
            $engine['model'] = $nsClass .'\\'. $element['models'][$model]['folder'] .'\\'. $model;
        
        //create the view
        if(count($element['views'][$view]) == 1)
            $engine['view'] = $nsClass .'\\'. $view;
        else
            $engine['view'] = $nsClass .'\\'. $element['views'][$view]['folder'] .'\\'. $view;
    ...
    /**
     * Processes the project XML and integrates the elemnts into the framework
     */
    ...
    public function getArguments(Request $request, $ctrl) {
     ...
     * @return type
     */
    public function processPanel($panel, $panelargs = array()){
        
        $element = self::_getFullElementByController($panel['controller']); 
        return $this->generatePartial($element,$panel['controller'],$panel['method'], $panel['panel'], $panelargs);
    }
    ...
    /**
     * Runs the main controler and its resolved arguments
     * @param type $controller
