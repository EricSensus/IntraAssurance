...
class AclBuilder {
    ...
    public function createGateway($element) {
        ...
        if($auth == Gateway::class){
            ...
            if($this->file->exists($this->mouldpath .DS. 'acl' .DS. 'gateway.mould')){
                ...
                if(array_key_exists($element, $this->elements)){
                    ...
                    $gateway = $this->file->get($this->mouldpath .DS. 'acl' .DS. 'gateway.mould');
    ...
    public function createRole($name, $element, $alias, $level = 0, $description = ''){
        ...
        $rolemould = $this->mouldpath .DS. 'acl' .DS. 'role.mould';
        ...
        if($this->file->exists($rolemould)){
            ...
            //save the role class
            if($this->file->put($aclpath .DS. ucfirst($alias).'.php', $role, true)){
