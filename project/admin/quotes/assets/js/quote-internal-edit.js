$(function () {
    var sub_btn = $('button[name=btnSubmit]');
    ...
    var Common = {
        ...
        setUrl: function () {
            ...
            main_form.attr('action', SITE_PATH + '/admin/myquote/save/' + PRODUCT);
        }
    };
    var Motor = {
    ...
    var Loader = {
        autoFill: function () {
            ...
            QUOTE_ID = customer_data.quote_id;
            Quote.preview();
        },
        prepAuto: function () {
            var el, elv, count, value = null;
            if (PRODUCT === 'motor') {
                el = $('[name=somecovers]');
                elv = $('[name=othercovers]');
                value = customer_data.somecovers;
                count = customer_data.othercovers;
            }
            if (PRODUCT === 'accident') {
                el = $('[name=other_covers]');
                elv = $('[name=howmany]');
                value = customer_data.other_covers;
                count = customer_data.howmany;
            }
            if (value === 'yes') {
                el.filter('[value=yes]').attr('checked', true).trigger('change');
                elv.val(count).trigger('change');
            }
